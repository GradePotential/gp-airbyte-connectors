#!/usr/bin/env bash
# Build and deploy source-mongodb-v2 (ScaleGrid build) to the Airbyte VM.
# Usage: ./scripts/deploy-mongodb.sh <version>   e.g. 1.0.6
#
...
IMAGE="airbyte/source-mongodb-v2-scalegrid:${VERSION}"
VM_SSH="${AIRBYTE_VM_USER}@${AIRBYTE_VM_IP}"

echo "==> Building JAR (skip tests)"
export PATH="/opt/homebrew/opt/openjdk@21/bin:$PATH"
cd "$ROOT_DIR"
./gradlew :sources:source-mongodb-v2:assemble -x test

echo "==> Building Docker image (linux/amd64)"
docker build \
  -f sources/source-mongodb-v2/Dockerfile.amd64 \
  --platform linux/amd64 \
  -t "$IMAGE" \
  sources/source-mongodb-v2/
...
ssh -i "$AIRBYTE_SSH_KEY" "$VM_SSH" \
  "docker exec ${AIRBYTE_K8S_CONTROL_PLANE} \
   ...
   \"UPDATE actor_definition_version SET docker_image_tag='${VERSION}' WHERE id='${MONGODB_ACTOR_DEF_VERSION_ID}';\""

echo "==> Done. source-mongodb-v2-scalegrid ${VERSION} deployed."
