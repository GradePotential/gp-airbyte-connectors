...
class MsSqlSourceOperations :
    ...
    /**
     * Override stream creation to advertise CT streams as having a source-defined cursor.
     * This prevents the Airbyte UI from requiring the user to pick a cursor field —
     * the CT version is managed internally as state.
     * Streams with a valid PK get INCREMENTAL support; others get FULL_REFRESH only.
     */
    override fun create(config: SourceConfiguration, discoveredStream: DiscoveredStream): AirbyteStream {
        ...
        val syncModes = if (hasPK) listOf(SyncMode.FULL_REFRESH, SyncMode.INCREMENTAL)
                        else listOf(SyncMode.FULL_REFRESH)
        val primaryKey = if (hasPK) discoveredStream.primaryKeyColumnIDs else emptyList()
        val isCtMode = config is MsSqlServerSourceConfiguration &&
            config.incrementalReplicationConfiguration is ChangeTrackingIncrementalConfiguration
        return io.airbyte.cdk.discover.AirbyteStreamFactory.createAirbyteStream(discoveredStream).apply {
            supportedSyncModes = syncModes
            sourceDefinedPrimaryKey = primaryKey
            // CT manages its own cursor via version numbers; no user-specified cursor needed.
            sourceDefinedCursor = hasPK
            isResumable = hasPK
            // Declare the synthetic soft-delete column so the destination materializes it as a
            // typed column. The field is populated at query time by MsSqlServerJdbcCtIncrementalPartition
            // (non-null for deletes, null for inserts/updates). Without this declaration, BigQuery
            // Destinations V2 silently drops the field because it isn't in the catalog schema.
            if (hasPK && isCtMode) {
                (jsonSchema?.get("properties") as? ObjectNode)?.set<JsonNode>(
                    MsSqlServerJdbcCtIncrementalPartition.DELETED_AT_FIELD,
                    Jsons.valueToTree(mapOf("type" to listOf("null", "string"))),
                )
            }
        }
    }
    ...
    // CT mode uses global = false, so globalCursor is null and globalMetaFields is empty.
    ...
    override fun decorateRecordData(
        ...
        // No-op: CT mode does not use global meta fields.
        // The _ab_cdc_deleted_at column is emitted directly by CHANGETABLE SQL.
    }
