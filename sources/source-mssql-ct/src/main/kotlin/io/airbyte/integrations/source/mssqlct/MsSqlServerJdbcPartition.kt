...
/**
 * Non-resumable full-table snapshot for CT mode on tables with no primary key.
 *
 * Unlike [MsSqlServerJdbcNonResumableSnapshotPartition], this saves a CT-compatible
 * [completeState] (i.e. `{ct_version, pk_name:null}`) so that [MsSqlServerJdbcPartitionFactory]
 * can detect "snapshot just finished this sync cycle" vs "new sync, re-run snapshot" by comparing
 * the stored CT version against the current database CT version.
 */
class MsSqlServerJdbcCtNonResumableSnapshotPartition(
    selectQueryGenerator: SelectQueryGenerator,
    streamState: DefaultJdbcStreamState,
    val capturedCtVersion: Long,
) : MsSqlServerJdbcPartition(selectQueryGenerator, streamState) {

    override val completeState: OpaqueStateValue =
        MsSqlServerJdbcCtStreamState.snapshotComplete(capturedCtVersion)
}
...
/**
 * Change Tracking snapshot partition. Performs the initial full-table read ordered by PK, while
 * storing the CT version captured at snapshot start in the state.
 *
 * When complete, the state transitions to CT incremental mode ([pkName] = null).
 */
class MsSqlServerJdbcCtSnapshotPartition(
    selectQueryGenerator: SelectQueryGenerator,
    streamState: DefaultJdbcStreamState,
    val primaryKey: Field,
    override val lowerBound: List<JsonNode>?,
    /** CT version captured before the snapshot started; persisted in every state checkpoint. */
    val capturedCtVersion: Long,
) :
    MsSqlServerJdbcResumablePartition(
        selectQueryGenerator, streamState, listOf(primaryKey)
    ) {
    override val upperBound: List<JsonNode>? = null

    override val completeState: OpaqueStateValue
        get() = MsSqlServerJdbcCtStreamState.snapshotComplete(capturedCtVersion)

    override fun incompleteState(lastRecord: ObjectNode): OpaqueStateValue {
        val pkVal = lastRecord[primaryKey.id]?.asText() ?: return completeState
        return MsSqlServerJdbcCtStreamState.snapshotCheckpoint(
            ctVersion = capturedCtVersion,
            pkName = primaryKey.id,
            pkVal = pkVal,
        )
    }
}
