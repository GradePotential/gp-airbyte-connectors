...
@JsonSchemaTitle("MSSQL Source Spec (Change Tracking)")
@JsonPropertyOrder(
...
@JsonSubTypes(
    ...
    JsonSubTypes.Type(value = ChangeTracking::class, name = "CHANGE_TRACKING"),
)
@JsonSchemaTitle("Update Method")
...
@JsonSchemaTitle("Read Changes using Change Tracking")
@JsonSchemaDescription(
    "Incrementally reads new inserts, updates, and deletes using SQL Server's " +
        "<a href=\"https://learn.microsoft.com/en-us/sql/relational-databases/track-changes/about-change-tracking-sql-server\">" +
        "Change Tracking</a> feature. " +
        "Supports SQL Server 2008+ including Web Edition. " +
        "Change Tracking must be enabled on the database and all tables to sync."
)
class ChangeTracking : IncrementalConfigurationSpecification {
    @JsonProperty("invalid_ct_version_behavior")
    @JsonSchemaTitle("Invalid CT Version Behavior (Advanced)")
    @JsonPropertyDescription(
        "Determines whether Airbyte should fail or re-sync data when the stored Change Tracking " +
            "version is older than the database's minimum valid version (i.e., the retention " +
            "window has been exceeded). 'Fail sync' requires manual reset. 'Re-sync data' " +
            "triggers a full re-sync automatically."
    )
    @JsonSchemaDefault("Re-sync data")
    @JsonSchemaInject(
        json = """{"order":1,"always_show":true, "enum": ["Fail sync","Re-sync data"]}"""
    )
    var invalidCtVersionBehavior: String? = "Re-sync data"

    @JsonProperty("initial_load_timeout_hours")
    @JsonSchemaTitle("Initial Load Timeout in Hours (Advanced)")
    @JsonPropertyDescription(
        "The maximum time allowed for the initial full-table snapshot before proceeding to " +
            "Change Tracking incremental reads."
    )
    @JsonSchemaDefault("8")
    @JsonSchemaInject(json = """{"order":2, "max": 24, "min": 1,"always_show": true}""")
...
class MicronautPropertiesFriendlyIncrementalConfigurationSpecification {
    var method: String = "CHANGE_TRACKING"
